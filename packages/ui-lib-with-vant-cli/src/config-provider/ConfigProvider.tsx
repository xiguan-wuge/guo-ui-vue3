...
function mapThemeVarsToCSSVars(themeVars: Record<string, Numeric>) {
  ...
  Object.keys(themeVars).forEach(key => {
    cssVars[`--g-${kebabCase(key)}`] = themeVars[key]
  })
  return cssVars
}
export default defineComponent({
  ...
  setup(props, {slots}) {
    ...
    if(inBrowser) {
      const addTheme = () => {
        document.documentElement.classList.add(`g-theme-${props.theme}`)
      }
      const removeTheme = (theme = props.theme) => {
        document.documentElement.classList.remove(`g-theme-${theme}`)
      }
